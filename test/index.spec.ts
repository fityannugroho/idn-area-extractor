...
describe('idnxtr', () => {
  ...
  describe('negative tests', () => {
    it('should throw error if data is invalid', async () => {
      ...
      await expect(
        idnxtr({ data: '' as DataEntity, filePath }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: ' ' as DataEntity, filePath }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'lorem' as DataEntity, filePath }),
      ).rejects.toThrow(errMsg);
    });

    it('should throw error if filePath is empty', async () => {
      await expect(idnxtr({ data: 'regencies', filePath: '' })).rejects.toThrow(
        "'filePath' is required",
      );

      await expect(
        idnxtr({ data: 'regencies', filePath: ' ' }),
    ...
    it('should throw error if filePath does not exists', async () => {
      ...
      await expect(
        idnxtr({ data: 'regencies', filePath: ` ${filePath}` }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath: `${filePath} ` }),
      ).rejects.toThrow(errMsg);
      ...
      await expect(
        idnxtr({ data: 'regencies', filePath: distPath }),
      ).rejects.toThrow(errMsg);
    });
    ...
    it('should throw error if compare is not boolean', async () => {
      await expect(
        idnxtr({
          data: 'regencies',
          filePath,
          compare: 1 as unknown as boolean,
        }),
      ...
      await expect(
        idnxtr({
          data: 'regencies',
          filePath,
          compare: 'true' as unknown as boolean,
        }),
      ).rejects.toThrow("'compare' must be a boolean");
    });
    ...
    it('should throw error if destination is exists but empty', async () => {
      await expect(
        idnxtr({ data: 'regencies', filePath, destination: '' }),
    ...
    it('should throw error if output contains forbidden characters for file name', async () => {
      await expect(
        idnxtr({ data: 'regencies', filePath, output: 'output.csv' }),
      ).rejects.toThrow("'output' contains forbidden character(s)");

      await expect(
        idnxtr({ data: 'regencies', filePath, output: 'new/data' }),
      ).rejects.toThrow("'output' contains forbidden character(s)");
    });
    ...
    it('should throw error if range format is invalid', async () => {
      ...
      await expect(
        idnxtr({ data: 'regencies', filePath, range: '' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: ' ' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: ' 1-2' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: '1-2 ' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: 'abc' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: '1-' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: '-2' }),
      ).rejects.toThrow(errMsg);
      await expect(
        idnxtr({ data: 'regencies', filePath, range: '1,' }),
      ).rejects.toThrow(errMsg);
      ...
      await expect(
        idnxtr({ data: 'regencies', filePath, range: '1-2-3' }),
    ...
    it('should throw error if saveRaw is not boolean', async () => {
      await expect(
        idnxtr({
          data: 'regencies',
          filePath,
          saveRaw: 1 as unknown as boolean,
        }),
      ...
      await expect(
        idnxtr({
          data: 'regencies',
          filePath,
          saveRaw: 'true' as unknown as boolean,
        }),
      ).rejects.toThrow("'saveRaw' must be a boolean");
    });
    ...
    it('should throw error if silent is not boolean', async () => {
      await expect(
        idnxtr({
          data: 'regencies',
          filePath,
          silent: 1 as unknown as boolean,
        }),
      ...
      await expect(
        idnxtr({
          data: 'regencies',
          filePath,
          silent: 'true' as unknown as boolean,
        }),
      ).rejects.toThrow("'silent' must be a boolean");
    });
  });
  ...
  describe('positive tests', () => {
    ...
    it('should create a new CSV file: regencies.csv', async () => {
      ...
      expect(extractTxtFileRows(resultPath).at(0)).toStrictEqual(
